...
import {
  ...
  useMedicationEntriesQuery,
} from '@/hooks/useMedications';
import { getApiErrorMessage } from '@/services/api-error';
import { useCycleStore } from '@/stores/cycle.store';
import type { MedicationEntry } from '@/types/api.types';
...
export default function MedicationsRoute() {
  ...
  const dayEntriesQuery = useMedicationEntriesQuery(dateKey);
  const recentEntriesQuery = useMedicationEntriesQuery();
  const createMedicationMutation = useCreateMedicationEntryMutation();
  const deleteMedicationMutation = useDeleteMedicationEntryMutation();
  ...
  async function handleSave() {
    ...
    const parsedDose = dose.trim();

    if (parsedName.length < 2 || parsedName.length > 80) {
      setFormError('Digite um nome entre 2 e 80 caracteres.');
      setFeedback(null);
      return;
    }

    if (parsedDose.length > 80) {
      setFormError('A dose deve ter no maximo 80 caracteres.');
      setFeedback(null);
      return;
    }
  ...
  function confirmDelete(entry: MedicationEntry) {
    Alert.alert(
      'Remover medicamento',
      `Remover ${formatMedication(entry)} de ${formatDate(entry.date)}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            void deleteMedicationMutation.mutateAsync(entry.id);
          },
        },
      ],
    );
      ...
      >
        ...
        <View style={styles.heroCard}>
          <View style={styles.heroIcon}>
            <Feather name="plus-circle" size={23} color={colors.cream} />
          </View>
        ...
        <View style={styles.formCard}>
          ...
          <AppText variant="caption" tone="muted" style={styles.hint}>
            A dose e opcional. Use como fizer sentido para sua rotina.
          </AppText>
        ...
        <View style={styles.section}>
          ...
          {dayEntriesQuery.isLoading ? (
            ...
            <MedicationList
              emptyText="Nenhum medicamento salvo para esse dia."
              entries={dayEntries}
        ...
        <View style={styles.section}>
          ...
          {recentEntriesQuery.isLoading ? (
            ...
            <MedicationList
              emptyText="Nenhum medicamento registrado ainda."
  ...
  if (entries.length === 0) {
    return (
      <View style={styles.emptyBlock}>
        <Feather name="plus-circle" size={18} color={colors.soft} />
      ...
      {entries.map((entry) => (
        <View key={entry.id} style={styles.entryRow}>
          <View style={styles.entryIcon}>
            <Feather name="plus-circle" size={15} color={colors.blue} />
          </View>
          <View style={styles.entryCopy}>
            <AppText variant="label">{entry.name}</AppText>
            <AppText variant="caption" tone="muted">
              {formatDose(entry.dose)} - {formatDate(entry.date)}
            </AppText>
          </View>
          <Pressable
            accessibilityRole="button"
...
function formatMedication(entry: MedicationEntry) {
  const dose = formatDose(entry.dose);

  return dose === 'Sem dose' ? entry.name : `${entry.name} - ${dose}`;
}

function formatDose(value: string | null) {
  const trimmed = value?.trim();

  return trimmed ? trimmed : 'Sem dose';
}
