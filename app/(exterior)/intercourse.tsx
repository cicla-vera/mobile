import { Feather } from '@expo/vector-icons';
import { router, useLocalSearchParams } from 'expo-router';
import { useEffect, useMemo, useState } from 'react';
import {
  ActivityIndicator,
  Alert,
  Pressable,
  RefreshControl,
  ScrollView,
  StyleSheet,
  View,
} from 'react-native';

import { AppText, Button, Screen } from '@/components/ui';
import { colors, radius, shadow, spacing } from '@/constants/theme';
import {
  useCreateIntercourseEntryMutation,
  useDeleteIntercourseEntryMutation,
  useIntercourseEntriesQuery,
} from '@/hooks/useIntercourse';
import { getApiErrorMessage } from '@/services/api-error';
import { useCycleStore } from '@/stores/cycle.store';
import type { IntercourseEntry } from '@/types/api.types';
import { formatCalendarHeading } from '@/utils/calendar';
import { parseDateKey } from '@/utils/date';

type ProtectionChoice = {
  detail: string;
  icon: keyof typeof Feather.glyphMap;
  label: string;
  value: boolean;
};

const protectionChoices: ProtectionChoice[] = [
  {
    value: true,
    label: 'Com protecao',
    detail: 'barreira ou metodo de protecao usado',
    icon: 'shield',
  },
  {
    value: false,
    label: 'Sem protecao',
    detail: 'registro importante para acompanhar riscos',
    icon: 'alert-circle',
  },
];

export default function IntercourseRoute() {
  const params = useLocalSearchParams<{ date?: string }>();
  const selectedDateKey = useCycleStore((state) => state.selectedDate);
  const selectDate = useCycleStore((state) => state.selectDate);
  const dateKey =
    typeof params.date === 'string' && isDateKey(params.date)
      ? params.date
      : selectedDateKey;
  const [isProtected, setIsProtected] = useState(true);
  const [formError, setFormError] = useState<string | null>(null);
  const [feedback, setFeedback] = useState<string | null>(null);

  useEffect(() => {
    if (dateKey !== selectedDateKey) {
      selectDate(dateKey);
    }
  }, [dateKey, selectDate, selectedDateKey]);

  const selectedDate = useMemo(() => parseDateKey(dateKey), [dateKey]);
  const dayEntriesQuery = useIntercourseEntriesQuery(dateKey);
  const recentEntriesQuery = useIntercourseEntriesQuery();
  const createIntercourseMutation = useCreateIntercourseEntryMutation();
  const deleteIntercourseMutation = useDeleteIntercourseEntryMutation();
  const dayEntries = dayEntriesQuery.data ?? [];
  const protectedCount = dayEntries.filter((entry) => entry.protected).length;
  const unprotectedCount = dayEntries.length - protectedCount;
  const isRefreshing =
    dayEntriesQuery.isRefetching || recentEntriesQuery.isRefetching;
  const loadError = dayEntriesQuery.error ?? recentEntriesQuery.error;

  function refresh() {
    void Promise.all([dayEntriesQuery.refetch(), recentEntriesQuery.refetch()]);
  }

  function selectProtection(value: boolean) {
    setIsProtected(value);
    setFormError(null);
    setFeedback(null);
  }

  async function handleSave() {
    setFormError(null);
    setFeedback(null);

    try {
      await createIntercourseMutation.mutateAsync({
        protected: isProtected,
        date: dateKey,
      });
      setFeedback('Registro salvo para este dia.');
    } catch (error) {
      setFormError(
        getApiErrorMessage(error, 'Nao foi possivel salvar o registro.'),
      );
    }
  }

  function confirmDelete(entry: IntercourseEntry) {
    Alert.alert(
      'Remover registro',
      `Remover ${getProtectionLabel(entry.protected)} de ${formatDate(
        entry.date,
      )}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Remover',
          style: 'destructive',
          onPress: () => {
            void deleteIntercourseMutation.mutateAsync(entry.id);
          },
        },
      ],
    );
  }

  return (
    <Screen padded={false}>
      <ScrollView
        style={styles.scroll}
        contentContainerStyle={[
          styles.content,
          { paddingBottom: spacing[8] + spacing[6] },
        ]}
        refreshControl={
          <RefreshControl
            refreshing={isRefreshing}
            onRefresh={refresh}
            tintColor={colors.blue}
          />
        }
        keyboardShouldPersistTaps="handled"
        showsVerticalScrollIndicator={false}
      >
        <View style={styles.header}>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Voltar"
            onPress={() => router.back()}
            style={styles.iconButton}
          >
            <Feather name="arrow-left" size={19} color={colors.ink} />
          </Pressable>
          <View style={styles.headerCopy}>
            <AppText variant="caption" tone="muted" style={styles.eyebrow}>
              Relacoes
            </AppText>
            <AppText variant="heading">
              {formatCalendarHeading(selectedDate)}
            </AppText>
          </View>
        </View>

        <View style={styles.heroCard}>
          <View style={styles.heroIcon}>
            <Feather name="heart" size={23} color={colors.cream} />
          </View>
          <View style={styles.heroCopy}>
            <AppText variant="heading" tone="cream">
              {dayEntries.length > 0
                ? `${dayEntries.length} no dia`
                : 'Sem registro'}
            </AppText>
            <AppText style={styles.heroText}>
              {dayEntries.length > 0
                ? `${protectedCount} com protecao, ${unprotectedCount} sem protecao`
                : 'Guarde esse dado junto aos sinais do ciclo, com privacidade.'}
            </AppText>
          </View>
        </View>

        {loadError ? (
          <View style={styles.notice}>
            <Feather name="alert-circle" size={18} color={colors.danger} />
            <AppText variant="caption" style={styles.noticeText}>
              {getApiErrorMessage(
                loadError,
                'Nao deu para carregar relacoes.',
              )}
            </AppText>
          </View>
        ) : null}

        <View style={styles.formCard}>
          <View style={styles.sectionHeader}>
            <AppText variant="label">Novo registro</AppText>
            <AppText variant="caption" tone="muted">
              protecao
            </AppText>
          </View>

          <View style={styles.choiceGrid}>
            {protectionChoices.map((choice) => (
              <Pressable
                key={choice.label}
                accessibilityRole="button"
                accessibilityState={{ selected: isProtected === choice.value }}
                onPress={() => selectProtection(choice.value)}
                style={[
                  styles.protectionChip,
                  isProtected === choice.value && styles.protectionSelected,
                ]}
              >
                <View
                  style={[
                    styles.protectionIcon,
                    isProtected === choice.value &&
                      styles.protectionIconSelected,
                  ]}
                >
                  <Feather
                    name={choice.icon}
                    size={16}
                    color={
                      isProtected === choice.value
                        ? colors.cream
                        : colors.blue
                    }
                  />
                </View>
                <View style={styles.protectionCopy}>
                  <AppText
                    variant="label"
                    tone={isProtected === choice.value ? 'cream' : 'ink'}
                  >
                    {choice.label}
                  </AppText>
                  <AppText
                    variant="caption"
                    tone={isProtected === choice.value ? 'cream' : 'muted'}
                  >
                    {choice.detail}
                  </AppText>
                </View>
              </Pressable>
            ))}
          </View>

          {formError ? (
            <AppText variant="caption" style={styles.formError}>
              {formError}
            </AppText>
          ) : null}
          {feedback ? (
            <AppText variant="caption" style={styles.feedback}>
              {feedback}
            </AppText>
          ) : null}

          <Button
            loading={createIntercourseMutation.isPending}
            disabled={createIntercourseMutation.isPending}
            onPress={() => void handleSave()}
            style={styles.saveButton}
          >
            Salvar registro
          </Button>
        </View>

        <View style={styles.section}>
          <SectionHeader
            title="Neste dia"
            detail={`${dayEntries.length} registros`}
          />
          {dayEntriesQuery.isLoading ? (
            <ActivityIndicator color={colors.blue} style={styles.loader} />
          ) : (
            <IntercourseList
              emptyText="Nenhum registro salvo para esse dia."
              entries={dayEntries}
              onDelete={confirmDelete}
            />
          )}
        </View>

        <View style={styles.section}>
          <SectionHeader title="Ultimos registros" detail="historico recente" />
          {recentEntriesQuery.isLoading ? (
            <ActivityIndicator color={colors.blue} style={styles.loader} />
          ) : (
            <IntercourseList
              emptyText="Nenhum registro criado ainda."
              entries={(recentEntriesQuery.data ?? []).slice(0, 6)}
              onDelete={confirmDelete}
            />
          )}
        </View>
      </ScrollView>
    </Screen>
  );
}

function SectionHeader({ title, detail }: { title: string; detail: string }) {
  return (
    <View style={styles.sectionHeader}>
      <AppText variant="label">{title}</AppText>
      <AppText variant="caption" tone="muted">
        {detail}
      </AppText>
    </View>
  );
}

function IntercourseList({
  emptyText,
  entries,
  onDelete,
}: {
  emptyText: string;
  entries: IntercourseEntry[];
  onDelete: (entry: IntercourseEntry) => void;
}) {
  if (entries.length === 0) {
    return (
      <View style={styles.emptyBlock}>
        <Feather name="heart" size={18} color={colors.soft} />
        <AppText variant="caption" tone="muted" style={styles.emptyText}>
          {emptyText}
        </AppText>
      </View>
    );
  }

  return (
    <View style={styles.entriesList}>
      {entries.map((entry) => (
        <View key={entry.id} style={styles.entryRow}>
          <View style={styles.entryIcon}>
            <Feather
              name={entry.protected ? 'shield' : 'alert-circle'}
              size={15}
              color={colors.blue}
            />
          </View>
          <View style={styles.entryCopy}>
            <AppText variant="label">
              {getProtectionLabel(entry.protected)}
            </AppText>
            <AppText variant="caption" tone="muted">
              {formatDate(entry.date)}
            </AppText>
          </View>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Remover registro"
            onPress={() => onDelete(entry)}
            style={styles.deleteButton}
          >
            <Feather name="trash-2" size={16} color={colors.danger} />
          </Pressable>
        </View>
      ))}
    </View>
  );
}

function getProtectionLabel(value: boolean) {
  return value ? 'Com protecao' : 'Sem protecao';
}

function formatDate(value: string) {
  const date = new Date(value);

  if (Number.isNaN(date.getTime())) {
    return value.slice(0, 10);
  }

  return date.toLocaleDateString('pt-BR', {
    day: '2-digit',
    month: 'short',
    year: 'numeric',
  });
}

function isDateKey(value: string) {
  return /^\d{4}-\d{2}-\d{2}$/.test(value);
}

const styles = StyleSheet.create({
  scroll: {
    flex: 1,
    backgroundColor: colors.cream,
  },
  content: {
    paddingHorizontal: spacing[5],
    paddingTop: spacing[5],
  },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[3],
  },
  iconButton: {
    width: 42,
    height: 42,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 21,
    backgroundColor: 'rgba(255, 255, 255, 0.72)',
  },
  headerCopy: {
    flex: 1,
  },
  eyebrow: {
    marginBottom: 2,
    textTransform: 'uppercase',
  },
  heroCard: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[4],
    marginTop: spacing[5],
    padding: spacing[5],
    borderRadius: radius.md,
    backgroundColor: colors.plum,
    shadowColor: shadow.color,
    shadowOffset: shadow.offset,
    shadowOpacity: 0.16,
    shadowRadius: shadow.radius,
    elevation: shadow.elevation,
  },
  heroIcon: {
    width: 58,
    height: 58,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 29,
    backgroundColor: colors.coral,
  },
  heroCopy: {
    flex: 1,
  },
  heroText: {
    marginTop: spacing[1],
    color: 'rgba(255, 245, 236, 0.76)',
  },
  notice: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[2],
    marginTop: spacing[4],
    padding: spacing[3],
    borderRadius: radius.sm,
    backgroundColor: 'rgba(180, 35, 66, 0.08)',
  },
  noticeText: {
    flex: 1,
    color: colors.danger,
  },
  formCard: {
    marginTop: spacing[5],
    padding: spacing[5],
    borderWidth: 1,
    borderColor: 'rgba(20, 16, 17, 0.08)',
    borderRadius: radius.md,
    backgroundColor: colors.white,
  },
  section: {
    marginTop: spacing[5],
    padding: spacing[5],
    borderWidth: 1,
    borderColor: 'rgba(20, 16, 17, 0.08)',
    borderRadius: radius.md,
    backgroundColor: colors.white,
  },
  sectionHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    gap: spacing[3],
    marginBottom: spacing[4],
  },
  choiceGrid: {
    gap: spacing[2],
  },
  protectionChip: {
    minHeight: 72,
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[3],
    padding: spacing[3],
    borderWidth: 1,
    borderColor: 'rgba(20, 16, 17, 0.08)',
    borderRadius: radius.sm,
    backgroundColor: colors.cream,
  },
  protectionSelected: {
    borderColor: colors.plum,
    backgroundColor: colors.plum,
  },
  protectionIcon: {
    width: 38,
    height: 38,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 19,
    backgroundColor: colors.shell,
  },
  protectionIconSelected: {
    backgroundColor: 'rgba(255, 245, 236, 0.18)',
  },
  protectionCopy: {
    flex: 1,
  },
  formError: {
    marginTop: spacing[4],
    color: colors.danger,
  },
  feedback: {
    marginTop: spacing[4],
    color: colors.blue,
  },
  saveButton: {
    alignSelf: 'stretch',
    marginTop: spacing[6],
  },
  loader: {
    marginVertical: spacing[4],
  },
  emptyBlock: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[3],
    padding: spacing[4],
    borderRadius: radius.sm,
    backgroundColor: colors.cream,
  },
  emptyText: {
    flex: 1,
  },
  entriesList: {
    gap: spacing[3],
  },
  entryRow: {
    minHeight: 62,
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[3],
    padding: spacing[3],
    borderRadius: radius.sm,
    backgroundColor: colors.cream,
  },
  entryIcon: {
    width: 36,
    height: 36,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 18,
    backgroundColor: colors.shell,
  },
  entryCopy: {
    flex: 1,
  },
  deleteButton: {
    width: 38,
    height: 38,
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 19,
    backgroundColor: 'rgba(180, 35, 66, 0.08)',
  },
});
